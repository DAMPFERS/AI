...
plt.plot(history.history['accuracy'], label='Training Accuracy')  # График точности на обучении
plt.plot(history.history['val_accuracy'], label='Validation Accuracy')  # График точности на валидации
plt.title('Accuracy')  # Заголовок
plt.xlabel('Epochs')  # Ось X
plt.ylabel('Accuracy')  # Ось Y
plt.legend()  # Легенда
plt.show()

# Визуализация функции потерь
plt.plot(history.history['loss'], label='Training Loss')  # График потерь на обучении
plt.plot(history.history['val_loss'], label='Validation Loss')  # График потерь на валидации
plt.title('Loss')  # Заголовок
plt.xlabel('Epochs')  # Ось X
...
# Сохранение модели
model.save('cross_or_line_classifier.keras')  # Сохраняем модель в формате Keras
